...
public class WordTree
{
	...
	public void insert(String word)
	{
	  ...
	  WordTreeNode child = root;
	  for(int i = temp.depth; i<word.length(); i++){
	    child = temp.createChild(word.charAt(i));
	    temp = child;
	  }
	...
	{  
		//   ADD YOUR CODE BELOW HERE
	  WordTreeNode temp = getPrefixNode(word);
	  if(temp.depth==word.length() && temp.toString().equals(word) && temp.endOfWord){
	    return true;
	  }
	...
	{
		//  ADD YOUR CODE BELOW 
	  WordTreeNode jam = getPrefixNode(prefix);
	  ArrayList<String> prefixList  = new ArrayList<String>();
	  if(jam.toString().equals(prefix)){
	    traverse(jam, prefixList);
	  }

		return prefixList;   //  REMOVE THIS STUB
		
		//  ADD YOUR CODE ABOVE HERE
	}
	private void traverse(WordTreeNode temp, ArrayList<String> list){
	  if(temp.isEndOfWord()){
	    list.add(temp.toString());
	  }
	  for (int i =0; i<256; i++){
	    if(temp.getChild((char )i )!= null){
	      traverse(temp.getChild((char)i), list);
	    }
	  }
	}
	...
	public class WordTreeNode
	{
		...
		{	   
			...
			// ADD YOUR CODE BELOW HERE
			child.parent =  this; //assign the parent of the child to this
			child.depth = this.depth +1; //the next node correspond to the child
			child.charInParent = c;     //character stored in charInparent
		...
		public String toString()
		{
			...
			// ADD YOUR CODE BELOW HERE
		  char[] storeWord = new char[this.depth];
		  WordTreeNode temp = this;

		  for(int i =this.depth-1; (i>0|| i ==0); i--){
		    storeWord[i] = temp.charInParent;
		    temp = temp.parent;
		    }
		  String results = "";
		  for(int i = 0; i<storeWord.length-1; i++){
		    results = results + storeWord[i];
		  }
		    

			return results + this.charInParent;   //  REMOVE THIS CODE STUB
